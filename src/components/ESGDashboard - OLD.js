...
Profits / donations given to charity,S,54.72,51.03,58.40,50.40,46.40,46.50,55.50,49.80,57.60,48.40,24.10,76.30,53.90,75.90,71.80`;
  ...
  return (
    <div className="p-4 max-w-6xl mx-auto">
      ...
      <div className="relative" style={{ height: `${filteredData.length * 40}px` }}>
        <AnimatePresence>
          {filteredData.map((item, index) => (
            ...
            >
              ...
              >
                <span className="absolute left-2 text-sm text-white">{item.name}</span>
                <span className="absolute right-2 text-sm text-white">{item.value.toFixed(1)}%</span>
              </div>
            </motion.div>
          ))}
        </AnimatePresence>
      ...
      <div className="mt-4">
        ...
        <ul className="list-disc pl-5">
          ...
          <li>{filteredData.filter(item => item.category === 'E').length} Environmental factors in top 10</li>
          <li>{filteredData.filter(item => item.category === 'S').length} Social factors in top 10</li>
          <li>{filteredData.filter(item => item.category === 'G').length} Governance factors in top 10</li>
        </ul>
      </div>
    </div>
  );
